...
# ==============================================================================
# Este script força a atualização mesmo se o git estiver quebrado ou bloqueado.
# ==============================================================================
...
# Forçar permissões caso o git esteja bloqueado
git config --global --add safe.directory "$INSTALL_DIR" || true

# Tentar limpar o lock do git se existir
rm -f .git/index.lock
...
# 6. Sincronização de Assets (Crucial para imagens aparecerem)
echo "🖼️ Sincronizando Assets para Produção..."
mkdir -p .output/public
cp -r public/* .output/public/ 2>/dev/null || true

# 7. Reinicialização PM2 (Matar versões antigas para evitar conflito)
echo "♻️ Reiniciando processo PM2..."
# Mata qualquer processo com o nome antigo ou novo
