...
export const Route = createFileRoute('/api/media/$name')({
  server: { handlers: { GET: async ({ request, params }) => {
    ...
    try {
      ...
      return new Response(file, { headers: { 'Content-Type': types[extname(name).toLowerCase()] ?? 'application/octet-stream', 'Cache-Control': 'private, max-age=3600', 'Accept-Ranges': 'bytes' } });
