...
export const startTrial = createServerFn({ method: 'POST' }).handler(async () => {
  ...
  return transaction(async (client) => {
    ...
    const inserted = await client.query<{ expires_at: Date }>("INSERT INTO subscriptions(user_id,type,status,expires_at) VALUES($1,'trial','active',now()+interval '30 minutes') RETURNING expires_at", [user.id]);
